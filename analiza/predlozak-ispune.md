# Predlozak ispune obrasca

## Osnovni podaci

- Ime i prezime / naziv: DALMACIJA EKO PROJEKT, poljoprivredna zadruga branitelja
- Adresa / sjediste: Srima IX 22, Srima (Grad Vodice)
- Telefon: +385 91 5247504
- E-mail adresa: iklaric@dalekopro.hr
- OIB: 34976906873
- MBS: 100015988
- MB / MBO: 02608537
- MIBPG: 207236
- Osoba ovlastena za zastupanje: Irena Klarić
- OIB osobe ovlastene za zastupanje: 45224556885

## Podaci o prijavi

- Tocka javnog poziva za koju se podnosi prijava:
- Ponudeni godisnji iznos zakupnine bez PDV-a (EUR):
- Rok na koji se podnosi prijava (godina):

## Dokumentacija koja se prilaze

- Potvrda o prebivalistu / izvadak iz registra: IZVADAK IZ SUDSKOG REGISTRA.pdf
- Potvrda Porezne uprave:
- Gospodarski program koristenja sumskog zemljista:
- Izjava o prihvacanju uvjeta javnog poziva:
- Potvrda o upisu u Upisnik poljoprivrednika:
- Potvrda o vlasnistvu ili posjedu stoke te broju uvjetnih grla:
- Potvrda o evidentiranim povrsinama u sustavu ARKOD:
- Ugovor o zakupu, ako se koristi:
- Dokaz o vlasnistvu ili posjedu zemljista, ako se koristi pravo prednosti:

## Radne napomene za ispunu

- Prijavitelj: DALMACIJA EKO PROJEKT, poljoprivredna zadruga branitelja
- Sjediste prijavitelja: Srima IX 22, Srima (Grad Vodice)
- OIB prijavitelja: 34976906873
- MBS prijavitelja: 100015988
- MIBPG prijavitelja: 207236
- Zastupnik za potpis: Irena Klarić
- Trazena tocka javnog poziva:
- Katastarske cestice:
- Ukupna povrsina:
- Pocetna cijena iz natjecaja:
- Planirani ponudeni iznos:
- Osnova prava prednosti, ako postoji:
- Dokaz dosadasnjeg zakupa, ako postoji:
- Lokalna povezanost sa podrucjem:
- Broj uvjetnih grla:
- ARKOD povrsine bitne za izracun:

## Zavrsni dio obrasca

- Mjesto:
- Datum:
- Potpis podnositelja zahtjeva:

## Napomena

Za svaku tocku javnog poziva koristi se zaseban obrazac i zasebna omotnica.
